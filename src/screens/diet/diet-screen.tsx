...
import { MealEntry as AIPlannedMeal } from '../../stores/ai-plan-store';
...
function mapMealTimingToType(timing: string): MealType | null {
  const normalized = timing.trim().toLowerCase();
  if (normalized.includes('아침') || normalized.includes('breakfast')) return 'breakfast';
  if (normalized.includes('점심') || normalized.includes('lunch')) return 'lunch';
  if (normalized.includes('저녁') || normalized.includes('dinner')) return 'dinner';
  if (normalized.includes('간식') || normalized.includes('snack')) return 'snack';
  return null;
}
...
function MealSection({
  ...
  entries: MealEntry[];
  onAdd: () => void;
  onOpenMenu: (entry: MealEntry) => void;
  onRecommend?: () => void;
    ...
    <AppCard style={styles.mealCard}>
      <View style={styles.mealHeader}>
        ...
        <View style={styles.mealActions}>
          {hasRecommendation && onRecommend ? (
            <TouchableOpacity style={[styles.recommendButton, { borderColor: colors.accent, backgroundColor: colors.accentMuted }]} onPress={onRecommend}>
              <Text style={[styles.recommendButtonText, { color: colors.accent, fontFamily: typography.fontFamily }]}>추천</Text>
            </TouchableOpacity>
          ) : null}
...
export default function DietScreen({ navigation }: Props) {
  ...
  const recommendedMealsByType = useMemo(() => {
    const mapped: Partial<Record<MealType, AIPlannedMeal>> = {};
    for (const meal of todayPlanDiet?.meals ?? []) {
      const type = mapMealTimingToType(meal.timing);
      if (type && !mapped[type]) {
        mapped[type] = meal;
      }
    }
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.mealSections}>
          {MEAL_ORDER.map(type => (
            <MealSection
              key={type}
              title={MEAL_TYPE_LABEL[type]}
              entries={grouped[type]}
              onAdd={() => navigation.navigate('FoodSearch', { mealType: type, date: today })}
              onOpenMenu={e => { setSelectedEntry(e); setEditingAmount(String(e.amount)); }}
              hasRecommendation={Boolean(recommendedMealsByType[type])}
              onRecommend={recommendedMealsByType[type] ? () => setRecommendedMeal(recommendedMealsByType[type] ?? null) : undefined}
            />
          ))}
        </View>
      </ScrollView>
      ...
      <Modal visible={Boolean(recommendedMeal)} transparent animationType="fade" onRequestClose={() => setRecommendedMeal(null)}>
        <TouchableWithoutFeedback onPress={() => setRecommendedMeal(null)}>
          <View style={styles.overlay}>
            <TouchableOpacity style={styles.backdrop} activeOpacity={1} onPress={() => setRecommendedMeal(null)} />
            <TouchableWithoutFeedback onPress={() => {}}>
              <AppCard style={styles.recommendSheet}>
                <Text style={[styles.sheetTitle, { color: colors.text, fontFamily: typography.fontFamily }]}>
                  {recommendedMeal?.timing ?? '추천 식단'}
                </Text>
                <Text style={[styles.recommendCalories, { color: colors.accent, fontFamily: typography.fontFamily }]}>
                  약 {recommendedMeal?.calories ?? 0} kcal
                </Text>
                <View style={styles.recommendFoodList}>
                  {(recommendedMeal?.foods ?? []).map((food, index) => (
                    <Text key={`${food}-${index}`} style={[styles.recommendFoodItem, { color: colors.textSecondary, fontFamily: typography.fontFamily }]}>
                      · {food}
                    </Text>
                  ))}
                </View>
                <Text style={[styles.recommendMacroLine, { color: colors.textTertiary, fontFamily: typography.fontFamily }]}>
                  단백질 {recommendedMeal?.macros.protein ?? 0}g · 탄수화물 {recommendedMeal?.macros.carbs ?? 0}g · 지방 {recommendedMeal?.macros.fat ?? 0}g
                </Text>
                <View style={styles.recommendFooter}>
                  <Button mode="contained" onPress={() => setRecommendedMeal(null)}>
                    확인
                  </Button>
                </View>
              </AppCard>
...
const styles = StyleSheet.create({
  ...
  recommendButtonText: { fontSize: 13, fontWeight: '700' },
  ...
  recommendCalories: { fontSize: 15, fontWeight: '700' },
  recommendFoodList: { gap: 6 },
  recommendFoodItem: { fontSize: 14, lineHeight: 20 },
  recommendMacroLine: { fontSize: 13, lineHeight: 19, marginTop: 4 },
  recommendFooter: { marginTop: 8, alignItems: 'flex-end' },
});
